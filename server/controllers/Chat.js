...
const makeChat = async (req, res) => {
  ...
  if(await Chat.findOne(query1) == null || await Chat.findOne(query2) == null)
  {
    return res.status(400).json({ error: 'Chat already exist' });
  }
  if(req.body.user1 == req.session.account.username)
  {
    return res.status(400).json({ error: 'Must message other user' });
  }
  if (!req.body.user1) {
